...
public class ModelReadingUtils { 
	 ...
	 */
	public static List<Literal> readLiteralInLang(Resource r, Property property, String lang) {
		if (r.hasProperty(property)) {
			if(lang != null) {
				// we asked for an explicit language
				if(r.listProperties(property, lang).toList().size() > 0) {
					return r.listProperties(property, lang).toList().stream()
							.map(s -> s.getObject().asLiteral())
							.collect(Collectors.toList());
				} else {
					// also look for values with no language
					return r.listProperties(property, "").toList().stream()
							.map(s -> s.getObject().asLiteral())
							.collect(Collectors.toList());
				}
			} else {
