...
public class SheetReader {
	...
	public static List<ColumnSpecification> buildColumnSpecifications(
		...
		for (PropertyShape pShape : propertyShapes) {
			...
			// find a language
			List<String> languageIn = readShLanguageIn(pShape);
			if (languageIn != null) {
				for (String dataLanguage : languageIn) {
	...
	public static List<String> readShLanguageIn(PropertyShape r) {
		if (r.getPropertyShape().hasProperty(SHACLM.languageIn)) {
			return r.getPropertyShape().getProperty(SHACLM.languageIn).getList().asJavaList().stream().map(node -> node.asLiteral().getLexicalForm()).collect(Collectors.toList());
		}
		return null;
	}
